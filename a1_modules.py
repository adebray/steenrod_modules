...
# M is an A(1)-module that the elements of s are pieces of
def apply_sq(i: int, s: set, M):
	sq = lambda x: x.sq1 if i == 1 else lambda x: x.sq2
	print(x.sq2 for x in s)
	return set(M[sq(x)] for x in s if x != O)
...
def elem_update(M, M_next, image, fr_tag):
	to_return = image
	fr = M_next[fr_tag]
	fr.in_kernel = (all(to == O for to in image))
	for to in image:
		if to != O:
			#print(fr_tag, '--> ', to)
			if to.is_hit_by:
...
def update_info(M, m_tag, M_next, new_A1_tag):
	...
	dupes_A1 = elem_update(M, M_next, apply_sq(1, dupes_A0, M), 'A1' + new_A1_tag)
	print('derp0')
	print(apply_sq(2, dupes_A0, M))
	print('derp')
	dupes_A2 = elem_update(M, M_next, apply_sq(2, dupes_A0, M), 'A2' + new_A1_tag)
	dupes_A3 = elem_update(M, M_next, apply_sq(1, dupes_A2, M), 'A3' + new_A1_tag)
	dupes_B3 = elem_update(M, M_next, apply_sq(2, dupes_A1, M), 'B3' + new_A1_tag)
	dupes_B4 = elem_update(M, M_next, apply_sq(1, dupes_B3, M), 'B4' + new_A1_tag)
	_ = elem_update(M, M_next, apply_sq(2, dupes_A3, M), 'B5' + new_A1_tag)
	_ = elem_update(M, M_next, apply_sq(2, dupes_B4, M), 'B6' + new_A1_tag)
